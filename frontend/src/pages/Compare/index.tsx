...
const Compare: FC = () => {
  ...
  const [secondFileId, setSecondFile] = useState<number | null>(null);
  const [
  ...
  const handleCompare = () => {
    ...
    const result = { firstFileId, secondFileId };
  ...
  return (
    <Space direction="vertical" style={{ margin: "16px" }}>
      <Space>
        <SelectFiles
          files={branch}
          onSelectFirstFile={setFirstFile}
          onSelectSecondFile={setSecondFile}
        />
        <Button
          type="primary"
          onClick={handleCompare}
          disabled={
            !(firstFileId && secondFileId) || firstFileId === secondFileId
          }
          loading={isLoading}
        >
          Сравнить
